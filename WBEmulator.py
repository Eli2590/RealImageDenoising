...
class WBEmulator:
    ...
    def rgbuv_hist(self, I):
        ...
        for i in range(3):  # for each histogram layer, do
            ...
            diff_u = np.abs(np.tile(Iu, (np.size(A), 1)).transpose() - np.tile(A, (np.size(Iu), 1)))
            diff_v = np.abs(np.tile(Iv, (np.size(A), 1)).transpose() - np.tile(A, (np.size(Iv), 1)))
            diff_u[diff_u >= (eps / 2)] = 0
            diff_u[diff_u != 0] = 1
            diff_v[diff_v >= (eps / 2)] = 0
            diff_v[diff_v != 0] = 1
            temp = (np.tile(Iy, (np.size(A), 1)) * diff_u.transpose())
    ...
    def generateWbsRGB(self, I, outNum=10):
        ...
        for i in range(len(inds)):  # for each of the retried training examples,
            ...
            mf = sum(np.reshape(np.tile(weightsH, (1, 27)),
                                (self.K, 1, 9, 3)) * self.mappingFuncs[(idH - 1) * 10 + ind, :])
...
def addWB(I, p):
    ...
    if prob < p:
        wbColorAug = WBEmulator()  # create an instance of the WB emulator
        newImg, _ = wbColorAug.generateWbsRGB(I, 1)
        return np.float32(newImg[:, :, :, 0])
    return I
